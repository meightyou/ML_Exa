...
wine <- read.csv("Wine.csv") # adapt the path
...
library(ggplot2)
wine.d <- dist(wine[,-12], method = "manhattan") # matrix of Manhattan distances 

wine.melt <- melt(as.matrix(wine.d)) # create a data frame of the distances in long format
head(wine.melt)

ggplot(data = wine.melt, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() 
...
```{r, echo = T, message = FALSE,warning=FALSE}
wine.hc <- hclust(wine.d, method = "complete")
plot(wine.hc, hang=-1)

plot(wine.hc, hang=-1)
rect.hclust(wine.hc, k=4)
...
```{r, echo = T, message = FALSE,warning=FALSE}
wine.comp <- data.frame(wine[,-12], Clust=factor(wine.clust), Id=row.names(wine))
wine.df <- melt(wine.comp, id=c("Id", "Clust"))
head(wine.df)

ggplot(wine.df, aes(y=value, group=Clust, fill=Clust)) +
  geom_boxplot() +
  facet_wrap(~variable, ncol=4, nrow=3)

```

### 2.4 Choice of the number of clusters
...
wine.km <- kmeans(wine[,-12], centers=2)
wine.km$cluster ## see also wine.km for more information about the clustering hence obtained

```

## 4 PAM and silhouette plot

```{r, echo = T, message = FALSE,warning=FALSE}
library(cluster)
wine.pam <- pam(wine[,-12], k=3)
wine.pam
plot(silhouette(wine.pam))

```

# Course 10 Principal Component Analysis
...
library(FactoMineR)
library(factoextra)
wine.pca <- PCA(wine[,-12], ncp = 11, graph = FALSE)
wine.pca
```

### 2.1 Circle of correlations and interpretation
...
### 2.2 Map of the individual and biplot

```{r, echo = T, message = FALSE,warning=FALSE}
## fviz_pca_ind(wine.pca) ## only the individuals
fviz_pca_biplot(wine.pca) ## biplot

```

### 2.3 How many dimensions

```{r, echo = T, message = FALSE,warning=FALSE}
fviz_eig(wine.pca, addlabels = TRUE, ncp=11)
library(gridExtra)
p1 <- fviz_pca_biplot(wine.pca, axes = 1:2) 
p2 <- fviz_pca_biplot(wine.pca, axes = 3:4) 
p3 <- fviz_pca_biplot(wine.pca, axes = 5:6) 
grid.arrange(p1, p2, p3, nrow = 2, ncol=2)

```

## 3 Using PCA to represent clustering results
...
# Course 11 Variable Importance

## 1.1 Applying ML models
...
carseats.tr <- Carseats[carseats_index,]
carseats.te <- Carseats[-carseats_index,]

# define a linear regression
carseats.lm <- lm(Sales~., data=carseats.tr)

# define a regression tree (you can also use `adabag::autoprune()`here
carseats.rt <- rpart(Sales~., data=carseats.tr)

# define a support-vector machine
carseats.svm <- svm(Sales ~ ., data=carseats.tr)
```

## 1.2 Creating an explain object
...
x_train <- select(carseats.tr, -Sales)
y_train <- pull(carseats.tr, Sales)

explainer_lm <- DALEX::explain(model = carseats.lm, 
                                 data = x_train, 
                                 y = y_train,
                                 label = "Linear Regression")

explainer_rt <- DALEX::explain(model = carseats.rt,
                               data = x_train,
                               y = y_train,
                               label = "Regression Tree")

explainer_svm <- DALEX::explain(model = carseats.svm,
                                data = x_train,
                                y = y_train,
...
## 1.3 Plotting the feature importance
...
# Course 12 Random Forest

## 1 Classification task with Random Forests

### 1.1 Data preparation for Random Forest
...
wine.tr <- wine[wine_index == 1, ]
wine.te <- wine[wine_index == 2, ]
...
wine.rf <- randomForest(quality~., data=wine.tr, ntree=1000, importance=TRUE)
wine.pred.rf <- predict(wine.rf, newdata=wine.te)

library(caret)
confusionMatrix(data=wine.pred.rf, reference = wine.te$quality)
```
...
## 2 Regression task with Random Forests

```{r, echo = T, message = FALSE,warning=FALSE}
library(dplyr)
library(magrittr)
real_estate_data <- read.csv("real_estate_data.csv")
...
restate.tr <- real_estate_data[restate_index == 1, ]
restate.te <- real_estate_data[restate_index == 2, ]

# apply the RF model as a regression
restate.rf <- randomForest(Price~., data=restate.tr, ntree=1000, importance=TRUE)
restate.pred.rf<-predict(restate.rf, newdata=restate.te)

# compute rmse and plot the results as well the VarImp
(rmse <- sqrt(mean((restate.te$Price - restate.pred.rf)^2)))
plot(restate.te$Price ~ restate.pred.rf)
abline(0,1)
varImpPlot(restate.rf)
importance(restate.rf)
